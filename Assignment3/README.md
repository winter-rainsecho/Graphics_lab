...
### 完成情况
完成 Bonus 2（5 分）：在 `Texture` 类中实现 `getColorBilinear(float u, float v)`，新增 `bilinear_fragment_shader` 调用该方法；为凸显效果，将 `spot_texture.png` 下采样到 **512×512** 生成 `spot_texture_small.png`，并提交最近邻与双线性采样结果及对比。
...
| 采样方式 | 使用纹理 | 渲染结果 | 视觉差异 |
|---|---|---|---|
| 最近邻 (Nearest) | 512×512 `spot_texture_small.png` | `images/spot_texture_nearest.png` | 牛斑、嘴部、眼部出现明显块状像素，锯齿感强 |
| 双线性 (Bilinear) | 512×512 `spot_texture_small.png` | `images/spot_texture_bilinear.png` | 颜色在纹素之间平滑过渡，块状感显著减弱 |

### 结论
在纹理分辨率远低于屏幕像素覆盖范围时（本例 512×512 纹理被拉伸到约 700×700 画面），**最近邻采样**会直接将单个纹素的颜色放大为屏幕上的大块像素，产生明显锯齿；**双线性插值**通过相邻四个纹素加权混合，把突变变成连续渐变，视觉质量更高。这是纹理放大（magnification）场景下最常见的反锯齿手段。
...
- `texture spot small` 使用 `spot_texture_small.png`（512×512）渲染最近邻采样，用于 Bonus2 对比。
- `bilinear spot` 使用 `spot_texture_small.png`（512×512）渲染双线性插值采样。
