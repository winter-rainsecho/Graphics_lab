...
{
    // 1. 获取屏幕空间顶点坐标，用于包围盒和重心坐标计算
    auto v = t.toVector4();

    // 2. 计算三角形包围盒 (Bounding Box)
    ...
    for (int x = min_x; x <= max_x; x++)
    {
        for (int y = min_y; y <= max_y; y++)
        {
            ...
            {
                // 3b. 计算屏幕空间重心坐标 (alpha, beta, gamma)
                auto [alpha, beta, gamma] = computeBarycentric2D(x+0.5, y+0.5, v.data());

                // 3c. 透视校正深度插值
                //     Z = 校正因子(插值后的1/w), zp = 用于Z-Buffer的NDC深度
                ...
                if (zp < depth_buf[ind])
                {
                    ...
                    auto interpolated_shadingcoords = (alpha * view_pos[0] / v[0].w()
                                                     + beta  * view_pos[1] / v[1].w()
                                                     + gamma * view_pos[2] / v[2].w()) * Z;

                    // 3f. 组装片元着色器payload，调用fragment_shader获取最终颜色
